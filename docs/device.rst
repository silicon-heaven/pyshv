...
(for physical devices). There is also :class:`shv.rpcapi.methods.SHVMethods` you
can add to allow easier method implementation.
...
.. warning::
   ...
   should use :class:`shv.rpcapi.SHVBase` or
   :class:`shv.rpcapi.client.SHVClient` and not
   :class:`shv.rpcapi.device.SHVDevice`, unless you really have physical device
   that this application manages.
...
:meth:`shv.rpcapi.methods.SHVMethods.method`. This is a decorated that can be
used only in classes based on :class:`shv.rpcapi.methods.SHVMethods`. It ensures
that method is correctly discoverable and callable. You don't have to check the
access level or presence of a user's ID as that is verified based on the
...
:meth:`shv.rpcapi.methods.SHVMethods.Method.signal` that can be called to send
that signal. Thus in case of demonstrated example you would use ``await
...
The :attr:`shv.rpcapi.SHVBase.Request.progress` should be updated in case
...
:class:`shv.rpcapi.methods.SHVMethods` provides convenient way to define them.
...
:class:`shv.rpcapi.methods.SHVMethods` and thus you can use it to implement
nodes and methods that are dynamic, but it is also the implementation way if you
decide not to use :class:`shv.rpcapi.methods.SHVMethods`.

Listing nodes
^^^^^^^^^^^^^

This is implementation of ``ls`` method. In :class:`shv.rpcapi.SHVBase` you need
to override :meth:`shv.rpcapi.SHVBase._ls` method and implement it. It needs to
return iterator over child nodes. Do not forget to also yield base
implementation with ``yield from super()._ls(path)``.
...
There is also an helper :meth:`shv.rpcapi.SHVBase._ls_node_for_path` to
implement :meth:`shv.rpcapi.SHVBase._ls` that you might want to use to not have
to write code that correctly splits paths to the nodes.


Listing methods
^^^^^^^^^^^^^^^

To implement ``dir`` method you need to override :meth:`shv.rpcapi.SHVBase._dir`
method and implement generator providing method descriptions. This method is
called only if :meth:`shv.rpcapi.SHVBase._ls` reports this node as existing one.
Do not forget to first yield base implementation with ``yield from
...
parameter, optional user's ID, and access level in
:class:`shv.rpcapi.SHVBase.Request`. The access level needs to be checked if
user has required access level. The implementation needs to return result or
raise exception (preferably one of :class:`shv.rpcdef.RpcError` children) or
